...
import { Main, RowHeader, RowBody, BodySectionsCol, BodyValuesCol } from './styles';
...
function CreateEvents({ onClose, isMobile, createEvent, setPage }) {
    ...
    const paintSectionsNumber = useCallback(() => {
        return <Container style={{ paddingLeft: '30px', paddingRight: '30px' }}>
            <Row style={{ border: '1px solid rgba(70, 156, 240, 0.3)', borderRadius: '7px' }}>
                {['CLIENT/CUSTOMER INFORMATION', 'VENUE/EVENT INFORMATION', 'CONTRACT'].map((sect, i) => {
                    return <Col key={sect} xs={isMobile > 600 ? 4 : 12} className='p-0' style={{ backgroundColor: `${activeSection === i ? '#E6F1FD' : ''}`, borderRadius: '7px' }}>
                        <div className="stepper stepper-links d-flex flex-column" id="kt_create_account_stepper">
                            <div className="stepper-nav">
                                <div className={`stepper-item ${activeSection === i ? 'current' : i < activeSection ? 'completed' : ''}`} data-kt-stepper-element="nav">
                                    <h3 className="stepper-title">{sect}</h3>
                                </div>
                            </div>
                        </div>
                    </Col>
    ...
    }, [contractState, eventsState.guests_number]);
    ...
    const paintSectionsComponent = useCallback(() => {
        switch (activeSection) {
            ...
            case 2:
                return <Contract
                    requiredError={contractRequired}
                    state={contractState}
                    handleChange={contractHandleChange}
                />;
            default: return
        }
    }, [activeSection, eventsState, customerInfo, contractState, requiredEvents, customerRequired, contractRequired, contractHandleChange, isMobile]);
    ...
    const changeSection = useCallback((backOrCon) => {
        if (backOrCon) {
            if (activeSection === 1) {
                ...
                ['name', 'event_date', 'event_start', 'guests_number'].forEach((el, i) => {
            ...
            setActiveSection(activeSection - 1);
            if (activeSection === 1) {
                setRequiredEvents([false, false, false, false, false, false, false]);
            }
            if (activeSection === 2) {
                setCustomerRequired([false, false, false, false, false, false, false]);
            }
            if (activeSection === 2) {
                setContractRequired([false, false, false, false, false, false, false]);
            }
        }
    }, [activeSection, eventsState, customerInfo, contractState, customerRequired, dispatch, requiredEvents]);
    ...
    const createEvenet = useCallback(() => {
        ...
        if (cloneContractRequired.includes(true)) {
            ...
            createEvent({
                ...
                payments: contractState.payments
            }, onClose, setPage);
        }
    }, [contractState, contractRequired, eventsState, createEvent, dispatch, onClose, setPage]);
                    ...
                    <Col xs={6} className="text-left">
                        {activeSection !== 0 &&
                            <button type="button"
                                ...
                                className={`btn ${isMobile > 650 ? 'btn-lg' : ''} btn-light-primary me-3`} data-kt-stepper-action="previous">
                                <BsArrowLeft className="mr-1" />
                                Back</button>
                        }</Col>
                    <Col xs={6} className="text-right">{
                        activeSection === 0 ? <button type="button" className={`btn ${isMobile > 650 ? 'btn-lg' : ''} btn-primary`}
...
//         return <Container style={{ paddingLeft: '30px', paddingRight: '30px' }}>
//             <Row style={{ border: '1px solid rgba(70, 156, 240, 0.3)', borderRadius: '7px' }}>
//                 {['VENUE/EVENT INFORMATION', 'CLIENT/CUSTOMER INFORMATION', 'CONTRACT'].map((sect, i) => {
...
//                                     <h3 class="stepper-title" style={{fontSize:'15px'}}>{sect}</h3>
//                                 </div>
//                             </div>
//                         </div>
//                     </Col>
